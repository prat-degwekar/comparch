...
with open('/home/ashok/project_iiitdm/acl1_1K.rl','r') as f1:
	lines1=f1.readlines();
	...
	#llp=0
	with open('acl1_1K.rl_trace','r') as f_tr:
...
#	print(len(cou_sip))
	...
	sum_1=sum(ak1)
	print(sum_1)
	for i in range(0,len(dip_trace)):
		cou_dip=[]
		cou_sip=[]
		cou_sport=[]
		cou_dport=[]
		for yu in range(0,len(tr_dip)):
			ax=int(dip_prefix[yu])
			if(int(dip_prefix[yu]) == 0):
				cou_dip.append(yu)
			
			elif(dip_trace[i] == tr_dip[yu][0:ax]):
				cou_dip.append(yu)
		ak11.append(len(cou_dip))	

		for xc in range(0,len(tr_sip)):
			if xc in cou_dip:
				bx=int(sip_prefix[xc])
				if(int(sip_prefix[xc]) == 0):
					cou_sip.append(xc)
				elif(sip_trace[i] == tr_sip[xc][0:bx]):
					cou_sip.append(xc)
		ak12.append(len(cou_sip))
