...
bool Xor(bool a, bool b) { return (a == b) ? false : true; }
...
public:
	...
	bool isx(int n) { return dcs[n]; }

	bool bit(int n) { return value[n]; }

	void setx(int n, bool val) { dcs[n] = val; }

	void setval(int n, bool val) { value[n] = val; }

	int getMaxPart() { return part1; }

	int getMinMaxPart() { return part2; }

	void setMaxPart(int p) { part1 = p; }

	void setMinMaxPart(int p) { part2 = p; }
	...
	int distance(rule a) {
		...
		for (int i = 0; i < 104; i++) {
			...
			/*if( Xor( value[i] , a.bit(i) ) )
											cout<<"d = "<<d<<" value[ "<<i<<" ] : "<<value[i]<<" ,
				 bit( "<<i<<" ) = "<<a.bit(i)<<" , dcs[ "<<i<<" ] = "<<dcs[i]<<" , isx(
				 "<<i<<" ) =
							 "<<a.isx(i)<<endl;
			*/
		}
...
int main() {
	...
	// );  rule r2(
	// "1010101010101010*****01010101*10101011011010***0101010101010101010***0101010101010101010101010101010****"
	// );
	getRules(5);
	cout << "rule 1 : ";
	for (int i = 0; i < 104; i++) {
	...
	for (vector<vector<int>>::iterator jt = parts.begin(); jt != parts.end();
			 jt++) // or auto i : parts
	{
		vector<int> jj = *jt;
		for (int it : *jt) {
...
void minm() {
	...
	for (int i = 1; i < 5; i++) {
		...
		// for( vector< vector <int> > :: iterator jt = parts.begin() ; jt !=
		// parts.end() ; jt++ , ii++ )	 //or auto i : parts	for( vector <int> jt
		// : parts )
		for (int j = 0; j < parts.size(); j++) {
			// for( vector <int> :: iterator it = jt.begin() ; it != jt.end() ; it++ )
			// //for( auto it : jt ) // int value  vector < int > jj = *jt;
			for (int it : parts[j])
				cout << "it = " << it << endl;
			for (int it : parts[j]) {
				int k = sets[i].distance(sets[it]);
				cout << " while adding, it = " << it << " , for rule : " << i
						 << " and k = " << k << " , while its size = " << parts[j].size()
						 << endl;
				if (k == 0) {
					flag = 1;
...
void MinMaxPartition(vector<vector<int>> parts) {
	for (int i = 0; i < parts.size(); i++) {
		...
		/// plan -> split every max vector into min vector of vectors, then remove
		/// the max vector and add new vector of vectors
		...
		for (vector<vector<int>>::iterator jt = newvect.begin();
				 jt != newvect.end(); jt++) // or auto i : parts
		{
			vector<int> jj = *jt;
			for (int it : *jt) {
				cout << " , " << it;
